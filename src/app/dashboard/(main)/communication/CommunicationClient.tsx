...
export default function CommunicationClient({ charities }: { charities: Charity[] }) {
  ...
  // Extract all unique service names, prioritizing pillars first
        ...
        <div className="overflow-x-auto w-full max-h-[70vh] scrollbar-thin">
          <table className="w-full text-right text-xs border-collapse">
            <thead className="bg-slate-50 dark:bg-slate-800/80 text-slate-600 dark:text-slate-300 border-b border-slate-200 dark:border-slate-700 sticky top-0 z-10 shadow-sm">
              <tr>
                <th className="px-4 py-4 font-bold text-xs whitespace-nowrap bg-slate-50 dark:bg-slate-800 sticky right-0 z-20 border-l border-slate-200 dark:border-slate-700 shadow-[2px_0_5px_rgba(0,0,0,0.05)] w-48">الجمعية</th>
                {uniqueServiceNames.map(name => (
                  <th key={name} className="px-4 py-4 font-bold text-xs whitespace-nowrap border-l border-slate-200 dark:border-slate-700 text-center min-w-[200px]">
                    {name}
                  </th>
                ))}
              </tr>
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
              {filteredCharities.length === 0 ? (
                <tr>
                  <td colSpan={uniqueServiceNames.length + 1} className="p-12 text-center text-slate-500 font-bold text-sm">
                ...
                filteredCharities.map(charity => (
                  <tr key={charity.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/30 transition-colors">
                    ...
                    <td className="px-4 py-4 align-middle bg-white dark:bg-slate-900 sticky right-0 z-10 border-l border-slate-200 dark:border-slate-700 shadow-[2px_0_5px_rgba(0,0,0,0.05)]">
                    ...
                    {uniqueServiceNames.map(svcName => {
                      ...
                      return (
                        <td key={svcName} className="px-4 py-3 align-middle border-l border-slate-200 dark:border-slate-700">
                          {!service ? (
                            ...
                            <div className="group/cell relative flex flex-col items-center justify-center p-2 rounded-lg hover:bg-slate-50 dark:hover:bg-slate-800/80 transition-all text-center min-h-[64px]">
