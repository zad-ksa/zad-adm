    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-slate-50 dark:bg-slate-900 font-sans transition-colors" dir="rtl">
        <div className="text-center bg-white dark:bg-slate-900 p-12 rounded-xl border border-slate-200 dark:border-slate-800 shadow-[0_1px_2px_rgb(15_23_42/0.04)] max-w-md w-full mx-4 flex flex-col items-center transition-colors">
          ...
          <h1 className="text-page font-semibold text-slate-800 dark:text-slate-100 mb-3 tracking-tight transition-colors">التقرير غير موجود</h1>
         ...
         <div className="p-8 md:p-10 relative z-10">
           <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
              <div className="flex items-center gap-5">
                <div className="w-16 h-16 bg-secondary/10 dark:bg-secondary/20 rounded-xl flex items-center justify-center text-secondary font-semibold text-stat border border-secondary/20 dark:border-secondary/30 shadow-inner transition-colors">
                  {response.charityName.substring(0, 1)}
                </div>
                <div>
                  <div className="flex items-center gap-2 mb-1.5">
                    <span className="text-caption bg-secondary/10 dark:bg-secondary/20 text-secondary font-semibold px-2.5 py-0.5 rounded uppercase tracking-wider transition-colors">
                       تقرير التحليل السداسي
                    </span>
                  </div>
                  <h1 className="text-page font-semibold text-slate-800 dark:text-slate-100 tracking-tight mb-2 transition-colors">{response.charityName}</h1>
                  <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-md bg-slate-50 dark:bg-slate-900/50 border border-slate-100 dark:border-slate-700 transition-colors">
                    <span className="text-caption text-slate-400 dark:text-slate-500 transition-colors">بواسطة:</span>
                    <span className="text-caption font-semibold text-slate-600 dark:text-slate-300 transition-colors">{response.authorizedTitle}</span>
                  </div>
                </div>
              </div>
              ...
              <div className="w-full md:w-auto bg-slate-50 dark:bg-slate-900/50 p-5 rounded-xl border border-slate-100 dark:border-slate-700 transition-colors">
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <div className="text-caption font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider transition-colors">تاريخ التعبئة</div>
                </div>
            ...
            >
              ...
              <div className="relative z-10">
                <div className="flex items-center gap-3 pb-5 mb-5 border-b border-slate-100 dark:border-slate-700 transition-colors">
                  ...
                  <h3 className={`font-semibold text-section ${meta.colorClass} tracking-tight leading-tight transition-colors`}>
                    {meta.title}
                  </h3>
                </div>
                  ...
                  {list.length > 0 ? (
                    list.map((item, idx) => (
                      <li key={idx} className={`flex gap-3 items-start p-3 rounded-xl ${meta.lightBgClass} transition-colors`}>
                        <span className={`w-6 h-6 rounded-lg ${meta.bgClass} ${meta.colorClass} flex items-center justify-center font-semibold text-caption shrink-0 mt-0.5 border ${meta.borderClass} transition-colors`}>
                          {idx + 1}
                        </span>
                        <span className="text-title font-medium text-slate-700 dark:text-slate-200 leading-relaxed pt-0.5 transition-colors">{item}</span>
                      </li>
                    ))
                  ) : (
                    <li className="text-slate-400 dark:text-slate-500 text-caption font-medium text-center py-8 bg-slate-50/50 dark:bg-slate-900/30 rounded-xl border border-slate-100 dark:border-slate-700/50 border-dashed transition-colors">
