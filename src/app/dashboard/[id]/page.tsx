        ...
        <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden mb-8">
           <div className="p-8">
             <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6">
                <div>
                  <h1 className="text-2xl font-bold text-slate-800 mb-2">{response.charityName}</h1>
                  <p className="text-xs text-slate-500">تم التقييم بواسطة: {response.authorizedName} ({response.authorizedTitle})</p>
                </div>
                
                <div className="mt-4 md:mt-0 text-center">
                  <div className="text-xs font-bold text-slate-400 mb-1">النتيجة العامة</div>
                  <div className={`px-4 py-2 rounded-xl text-lg font-bold border
                    ${
                      response.scorePercentage >= 80 ? "bg-green-50 text-green-700 border-green-200" :
                      response.scorePercentage >= 60 ? "bg-blue-50 text-blue-700 border-blue-200" :
                      response.scorePercentage >= 40 ? "bg-orange-50 text-orange-700 border-orange-200" :
             ...
             <div className="grid grid-cols-2 md:grid-cols-4 gap-4 border-t border-slate-100 pt-6">
                <div>
                  <div className="text-xs text-slate-400 mb-1">تاريخ التأسيس</div>
                  <div className="font-bold text-slate-700 text-sm">{response.establishmentDate}</div>
                </div>
                <div>
                  <div className="text-xs text-slate-400 mb-1">رقم التصريح</div>
                  <div className="font-bold text-slate-700 text-sm">{response.licenseNumber}</div>
                </div>
                <div>
                  <div className="text-xs text-slate-400 mb-1">تاريخ الإرسال</div>
            ...
            <div key={i} className="bg-white rounded-xl border border-slate-200 overflow-hidden">
              ...
              <div className="bg-slate-50 px-6 py-4 border-b border-slate-200 flex justify-between items-center">
                ...
                <div className={`px-3 py-1 rounded-full text-xs font-bold border
                  ${
                    sec.percentage >= 80 ? "bg-green-50 text-green-700 border-green-100" :
                    sec.percentage >= 60 ? "bg-blue-50 text-blue-700 border-blue-100" :
                    sec.percentage >= 40 ? "bg-orange-50 text-orange-700 border-orange-100" :
