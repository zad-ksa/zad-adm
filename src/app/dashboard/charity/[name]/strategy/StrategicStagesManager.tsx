...
export default function StrategicStagesManager({ charityId, initialStages }: { charityId: string, initialStages: Stage[] }) {
  ...
  const handleAdd = () => {
    ...
    startTransition(async () => {
      // Optimistic locally
      const optimisticStage = { id: Math.random().toString(), name: newName, order: stages.length, isCurrent: false };
      setStages([...stages, optimisticStage]);
      setIsAdding(false);
      setNewName("");
      
      await addStrategicStage(charityId, newName);
  ...
  const handleUpdate = (id: string) => {
    ...
    startTransition(async () => {
      ...
      setEditingId(null);
      await updateStrategicStage(id, editName);
    });
  };
              ...
              <div className="flex-1">
                {editingId === stage.id ? (
                  ...
                  <div className="flex flex-col">
                     ...
                     {isCurrent && <span className="text-xs text-primary mt-1 font-medium bg-primary/10 w-fit px-2 py-0.5 rounded-full">المرحلة الحالية</span>}
                  </div>
                )}
              </div>
