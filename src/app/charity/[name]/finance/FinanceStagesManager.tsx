...
import { Coins, Check, X, Edit2, Trash2, Plus, ArrowUp, ArrowDown, Loader2, Settings } from "lucide-react";
...
export default function FinanceStagesManager({ 
  ...
  const handleAdd = () => {
    ...
    startTransition(async () => {
      ...
      await addFinanceStage(charityId, newName, "", newDescription, newStartDate, newEndDate);
  ...
  const handleUpdate = (id: string) => {
    ...
    startTransition(async () => {
      ...
      setEditingId(null);
      await updateFinanceStage(id, editName, "", editDescription, editStartDate, editEndDate);
    });
  };
      ...
      <div className="p-6 border-b border-slate-100 dark:border-slate-700 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <button 
            ...
            className="p-1.5 text-slate-400 hover:text-amber-500 bg-slate-50 hover:bg-amber-500/10 rounded-lg transition-colors"
        ...
        <button
          ...
          className="flex items-center justify-center gap-2 bg-amber-500/10 hover:bg-amber-500/20 text-amber-600 px-4 py-2 rounded-xl text-sm font-semibold transition-colors"
          disabled={isPending}
        >
                ...
                {editingId === stage.id ? (
                  <div className="flex flex-col gap-3">
                    ...
                    <textarea 
                      ...
                      rows={2}
                      className="w-full border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-800 text-slate-800 dark:text-slate-100 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-primary/50 outline-none resize-none"
                    />
          ...
          <div className="flex items-start gap-4 p-4 rounded-xl border border-primary border-dashed bg-primary/5">
            ...
            <div className="flex-1 flex flex-col gap-3">
              ...
              <textarea 
                ...
                placeholder="وصف مختصر للمرحلة (اختياري)"
                rows={2}
                className="w-full border border-primary/30 bg-white dark:bg-slate-800 text-slate-800 dark:text-slate-100 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-primary/50 outline-none resize-none"
              />
              <div className="flex flex-col sm:flex-row gap-4 sm:gap-3">
                <div className="w-full sm:w-1/2">
                  <label className="block text-xs font-bold text-slate-500 dark:text-slate-400 mb-1">تاريخ البداية (اختياري)</label>
