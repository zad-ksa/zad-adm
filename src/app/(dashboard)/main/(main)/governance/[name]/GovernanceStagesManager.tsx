...
export default function GovernanceStagesManager({ 
  ...
  const { isExpanded, toggle: toggleExpanded, expand: expandPanel } = useServiceAccordion("governance");
  ...
  const handleAdd = () => {
    ...
    startTransition(async () => {
      ...
      await addGovernanceStage(charityId, newName, "", newDescription, newStartDate, newEndDate, newIsContinuous, true);
      router.refresh();
    });
  };
  ...
  const handleUpdate = (id: string) => {
    ...
    startTransition(async () => {
      ...
      setEditingId(null);
      await updateGovernanceStage(id, editName, "", editDescription, editStartDate, editEndDate, editIsContinuous, stageToUpdate.isActive);
    });
  };
  ...
  const handleDeleteConfirm = () => {
    ...
    startTransition(async () => {
      await deleteGovernanceStage(stageId);
    });
  };
  ...
  const handleToggleActive = (id: string, currentActive: boolean) => {
    ...
    startTransition(async () => {
      await toggleActiveGovernanceStage(id, !currentActive);
    });
  };
      ...
      <div className="mt-2 border-t border-slate-100 dark:border-slate-700 pt-2 px-2 sm:px-3 pb-3">
        <InteractiveTimelineEditor
          ...
          onAdd={(stage) => {
            ...
            setStages([...stages, optimisticStage]);
            startTransition(async () => {
              await addGovernanceStage(charityId, stage.name, stage.duration || "", stage.description || "", stage.startDate ? stage.startDate.toISOString() : "", stage.endDate ? stage.endDate.toISOString() : "", stage.isContinuous, true);
            });
          }}
          onUpdate={(id, updates) => {
