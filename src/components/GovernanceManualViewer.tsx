        ...
        <div className="relative z-10 flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
          ...
          <form onSubmit={handleRevenueSubmit} className="flex w-full md:w-auto gap-3">
            <input 
              ...
              className="flex-1 md:w-64 px-4 py-3 bg-slate-50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-800 rounded-lg focus:ring-2 focus:ring-primary focus:border-primary outline-none text-slate-900 dark:text-white transition-all font-medium placeholder:text-slate-400"
              required
            />
            <button 
              type="submit" 
      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-6 auto-rows-[minmax(180px,auto)]">
        {sizes.map((size) => (
          ...
          >
            ...
            <div className="w-10 h-10 rounded-lg bg-slate-50 dark:bg-slate-900 border border-slate-100 dark:border-slate-800 flex items-center justify-center group-hover:scale-110 group-hover:bg-primary/5 group-hover:border-primary/20 transition-all duration-300 relative z-10">
              {size.icon}
            </div>
