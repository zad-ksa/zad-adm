...
export default async function VisionMissionDashboardPage({ params }: { params: Promise<{ name: string }> }) {
  ...
  if (!charity) {
    return <div className="p-8 text-center text-red-500 font-semibold">الجمعية غير موجودة</div>;
  }
      ...
      <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 p-6 md:p-8 shadow-[0_1px_2px_rgb(15_23_42/0.04)] transition-colors">
        <div className="flex items-center gap-3 mb-8 border-b border-slate-100 dark:border-slate-700 pb-4">
          ...
          <div>
            ...
            <p className="text-caption text-slate-500 mt-1">تجميع وتحليل مرئيات المشاركين حول صياغة التوجه الاستراتيجي للجمعية</p>
          </div>
        </div>
        ...
        {responses.length === 0 ? (
          <div className="text-center py-16 text-slate-500 dark:text-slate-400 bg-slate-50 dark:bg-slate-900/30 rounded-xl border border-dashed border-slate-200 dark:border-slate-800">
            ...
            <p className="font-semibold text-section text-slate-700 dark:text-slate-300">لم يقم أي مشارك بتعبئة الاستبيان بعد</p>
            <p className="text-caption text-slate-400 mt-1 max-w-sm mx-auto">يمكنك مشاركة الرابط أعلاه مع الموظفين وأعضاء مجلس الإدارة لجمع مشاركاتهم وتطلعاتهم.</p>
          </div>
        ) : (
