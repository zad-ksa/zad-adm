          ...
          <form id="create-schedule-form" onSubmit={handleSubmit} className="space-y-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-2">
                <label className="block text-caption font-semibold text-slate-700 dark:text-slate-300">اسم الاجتماع</label>
                <input 
                  type="text" 
                  required
                  value={title}
                  ...
                  className="w-full px-4 py-3 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-lg focus:ring-2 focus:ring-primary focus:border-primary outline-none transition-all text-caption font-medium"
                />
              </div>

              <div className="space-y-2">
                <label className="block text-caption font-semibold text-slate-700 dark:text-slate-300">مدة الاجتماع (بالدقائق)</label>
                <div className="relative">
                  ...
                  <input 
                    ...
                    className="w-full pl-4 pr-10 py-3 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-lg focus:ring-2 focus:ring-primary focus:border-primary outline-none transition-all text-caption font-medium"
                  />
                </div>
              </div>
            </div>
            ...
            <div className="bg-slate-50 dark:bg-slate-950 p-5 rounded-xl border border-slate-200 dark:border-slate-800 space-y-4">
              <h3 className="text-caption font-semibold text-slate-800 dark:text-slate-200 flex items-center gap-2">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="space-y-1">
                  <label className="text-caption font-semibold text-slate-500 uppercase">من تاريخ</label>
                  <input 
                    type="date" 
                    required
                    value={startDate}
                    onChange={e => { setStartDate(e.target.value); setHasGenerated(false); }}
                    className="w-full px-3 py-2.5 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-lg focus:ring-1 focus:ring-primary outline-none text-caption font-medium"
                  />
                </div>
                <div className="space-y-1">
                  <label className="text-caption font-semibold text-slate-500 uppercase">إلى تاريخ</label>
                  <input 
                    type="date" 
                    required
                    value={endDate}
                ...
                <div className="space-y-1">
                  <label className="text-caption font-semibold text-slate-500 uppercase">وقت البدء (يومياً)</label>
                  <input 
                    type="time" 
                    required
                    value={startTime}
                    onChange={e => { setStartTime(e.target.value); setHasGenerated(false); }}
                    className="w-full px-3 py-2.5 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-lg focus:ring-1 focus:ring-primary outline-none text-caption font-medium font-mono"
                  />
                </div>
                <div className="space-y-1">
                  <label className="text-caption font-semibold text-slate-500 uppercase">وقت الانتهاء (يومياً)</label>
                  <input 
                    type="time" 
                    required
                    value={endTime}
              ...
              <button
                ...
                className="w-full mt-2 bg-slate-900 dark:bg-white text-white dark:text-slate-900 py-3 rounded-lg font-semibold hover:opacity-90 transition-opacity flex items-center justify-center gap-2"
              >
            ...
            <label className="flex items-start gap-3 bg-slate-50 dark:bg-slate-950 p-4 rounded-xl border border-slate-200 dark:border-slate-800 cursor-pointer">
              ...
              <span className="text-caption">
                ...
                <span className="block text-caption text-slate-500 dark:text-slate-400 mt-1 leading-relaxed">
            ...
            {hasGenerated && (
              <div className="space-y-4 animate-in fade-in duration-300">
                <h3 className="text-caption font-semibold text-slate-800 dark:text-slate-200 mb-2">
                ...
                <div className="space-y-4">
                  {generatedDays.map((day, dIdx) => (
                    <div key={dIdx} className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-4">
                      <div className="font-semibold text-slate-700 dark:text-slate-300 mb-3 text-caption flex items-center gap-2">
                        <Calendar className="w-4 h-4 text-slate-400" />
                        {weekdayName(day.date)} - {day.date}
                      </div>
                      
                      {day.slots.length === 0 ? (
                        <p className="text-caption text-slate-400">لا يوجد أوقات متاحة في هذا اليوم.</p>
                      ) : (
                        <div className="flex flex-wrap gap-2">
                          {day.slots.map((slot, sIdx) => (
                            <button
                              ...
                              className="group flex items-center gap-1.5 px-3 py-1.5 bg-primary/10 hover:bg-red-50 dark:hover:bg-red-950/30 text-primary hover:text-red-600 rounded-lg text-caption font-semibold font-mono transition-colors border border-primary/20 hover:border-red-200 dark:hover:border-red-900/50"
            ...
            {error && (
              <div className="bg-red-50 dark:bg-red-900/20 text-red-600 dark:text-red-400 p-4 rounded-xl text-caption font-medium border border-red-100 dark:border-red-900/50">
                {error}
              </div>
            )}
          </form>
