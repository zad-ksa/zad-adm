...
const HexagonalSkeleton = () => (
  <div className="w-full space-y-8 animate-pulse font-sans" dir="rtl">
    ...
    <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-100 dark:border-slate-700 p-8 md:p-10 relative overflow-hidden">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        <div className="flex items-center gap-5">
          <div className="w-16 h-16 bg-slate-100 dark:bg-slate-700 rounded-2xl shrink-0"></div>
          <div className="space-y-3">
            <div className="h-4 bg-slate-100 dark:bg-slate-700 rounded-md w-28"></div>
            <div className="h-6 bg-slate-200 dark:bg-slate-600 rounded-md w-48 sm:w-64"></div>
            <div className="h-3 bg-slate-100 dark:bg-slate-700 rounded-md w-20"></div>
          </div>
        </div>
        <div className="w-full md:w-48 bg-slate-50 dark:bg-slate-900/50 p-5 rounded-xl border border-slate-100 dark:border-slate-700 space-y-2">
          <div className="h-3 bg-slate-100 dark:bg-slate-700 rounded-md w-16"></div>
          <div className="h-4 bg-slate-200 dark:bg-slate-600 rounded-md w-24"></div>
        </div>
      </div>
    </div>
    ...
    <div className="flex items-center gap-3 mb-8 pb-4 border-b border-slate-200 dark:border-slate-700">
      ...
      <div className="h-6 bg-slate-200 dark:bg-slate-600 rounded-md w-56"></div>
    </div>

    {/* Grid of 6 dimensions skeleton */}
    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
      {[1, 2, 3, 4, 5, 6].map((i) => (
        <div key={i} className="bg-white dark:bg-slate-800 rounded-2xl p-8 border border-slate-100 dark:border-slate-700 space-y-5">
          ...
          <div className="space-y-3">
            <div className="h-10 bg-slate-50 dark:bg-slate-900/50 rounded-xl w-full"></div>
            <div className="h-10 bg-slate-50 dark:bg-slate-900/50 rounded-xl w-full"></div>
            <div className="h-10 bg-slate-50 dark:bg-slate-900/50 rounded-xl w-full"></div>
          </div>
        </div>
...
export default function HexagonalLayoutClient({
  ...
  const sidebarContent = (
    <div className="bg-white dark:bg-slate-800 flex flex-col h-full border-l border-slate-200/80 dark:border-slate-700/80 shadow-[4px_0_24px_rgba(0,0,0,0.02)] relative transition-all duration-300">

      {/* Desktop Toggle Button */}
      <button
        onClick={() => setIsOpen(!isOpen)}
        className="hidden lg:flex absolute top-8 -left-3 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-400 dark:text-slate-500 hover:text-primary hover:border-primary/30 rounded-full w-6 h-6 items-center justify-center z-50 transition-all shadow-sm cursor-pointer"
      >
        <ChevronRight className={`w-4 h-4 transition-transform duration-300 ${!isOpen ? "rotate-180" : ""}`} />
      </button>

      {/* Header / Logo */}
      <div className={`flex items-center ${isOpen ? "justify-start px-6" : "justify-center px-0"} h-24 border-b border-slate-100 dark:border-slate-700 shrink-0 transition-all`}>
        {isOpen ? (
      ...
      <div className="lg:hidden absolute top-6 left-6 z-50">
        <button onClick={() => setIsOpen(false)} className="text-slate-400 dark:text-slate-500 hover:text-slate-600 dark:hover:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-700 p-2 rounded-lg transition-colors cursor-pointer">
          <X className="w-5 h-5" />
        </button>
      </div>
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar py-6 flex flex-col">
        ...
        <div className={`flex flex-col ${isOpen ? "items-start px-6" : "items-center px-2"} mb-8 pb-6 border-b border-slate-100 dark:border-slate-700 transition-all overflow-hidden`}>
          {logoUrl ? (
            <div className={`rounded-2xl overflow-hidden border border-slate-150 dark:border-slate-700 bg-white dark:bg-slate-800 flex items-center justify-center mb-3 shrink-0 transition-all ${isOpen ? "w-14 h-14" : "w-10 h-10"}`}>
          ...
          {responses.map((item) => {
            ...
            return (
              ...
              >
                ...
                <User className={`w-5 h-5 shrink-0 transition-all ${isOpen ? "ml-3" : "ml-0"} ${isActive ? "text-white" : "text-slate-400 dark:text-slate-500 group-hover:text-primary"}`} />
        ...
        <div className="mt-auto px-3 pt-6 border-t border-slate-100 dark:border-slate-700 shrink-0">
          <Link
  ...
  return (
    <div className="flex h-screen bg-slate-50 dark:bg-slate-900 overflow-hidden font-sans" dir="rtl">
      ...
      <div className="flex-1 flex flex-col min-w-0 h-full overflow-hidden relative">
        {/* Mobile Header */}
        <div className="lg:hidden bg-white dark:bg-slate-800 border-b border-slate-200 dark:border-slate-700 h-16 flex items-center justify-between px-4 shrink-0 z-10 shadow-sm">
          <div className="flex items-center gap-3">
            <button
              onClick={() => setIsOpen(true)}
              className="p-2 -mr-2 text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-700 rounded-lg transition-colors cursor-pointer"
            >
