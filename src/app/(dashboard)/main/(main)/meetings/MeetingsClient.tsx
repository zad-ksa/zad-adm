  ...
  if (selectedCategory === null) {
    return (
      <div className="space-y-3">
        ...
        <button 
          onClick={openCreate}
          className="fixed bottom-6 left-6 lg:bottom-8 lg:left-8 z-40 flex items-center gap-2 bg-primary hover:bg-primary/90 text-white px-5 py-3.5 rounded-full shadow-lg shadow-primary/30 transition-all hover:-translate-y-1 hover:shadow-xl hover:shadow-primary/40 font-semibold"
        >
          <Plus className="w-5 h-5" />
          <span className="text-caption">محضر جديد</span>
        </button>
      </div>
    );
  }
    ...
    <div className="space-y-3 animate-in fade-in slide-in-from-bottom-4 duration-500" dir="rtl">
      ...
      <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 p-3 space-y-2">
        ...
        <div className="relative">
          ...
          <input
            ...
            className="w-full border border-slate-200 dark:border-slate-700 rounded-lg pr-9 pl-3 py-1.5 text-caption bg-slate-50 dark:bg-slate-900 text-slate-800 dark:text-slate-100 focus:outline-none focus:ring-1 focus:ring-primary"
          />
        </div>
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <div className="flex items-center gap-1.5 mr-auto">
            <span className="text-caption text-slate-400">{filteredMeetings.length} من {meetings.length}</span>
            {activeFilters > 0 && (
              <button onClick={resetFilters} className="flex items-center gap-1 text-caption font-semibold text-red-400 hover:text-red-600 transition-colors">
        ...
        <div className="flex items-center gap-1.5 flex-wrap pt-1 border-t border-slate-100 dark:border-slate-700/50">
          <span className="text-caption font-semibold text-slate-400 shrink-0">الفترة:</span>
          {([
            { value: "", label: "الكل" },
            ...
            <button
              ...
              className={`px-2.5 py-0.5 rounded-full text-caption font-semibold transition-colors ${
      ...
      {meetings.length === 0 ? (
        <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 p-16 text-center">
          ...
          <p className="text-slate-400 dark:text-slate-500 text-caption">لا توجد محاضر بعد</p>
        </div>
        ...
        <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 p-10 text-center">
          ...
          <p className="text-slate-400 dark:text-slate-500 text-caption">لا توجد محاضر تطابق التصفية الحالية</p>
          <button onClick={resetFilters} className="mt-2 text-caption text-primary hover:underline">مسح التصفية</button>
        </div>
      ) : (
...
<div className="flex-1 overflow-auto p-4">
              <pre className="whitespace-pre-wrap font-sans text-caption leading-relaxed text-slate-700 dark:text-slate-200 text-right" dir="rtl">
