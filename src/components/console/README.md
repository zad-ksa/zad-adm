...
**٧. الأرقام في `StatStrip` لا بطاقاتٍ ملوّنة.** اللون فيه حالةٌ لا زينة:
`dot` أخضر (`active`) للمنجز، وكهرماني (`warn`) لما ينتظر، وأحمر (`danger`)
لما تأخّر — ورقمٌ لا يقول شيئاً عن حالةٍ بلا نقطة. والوحدة («ريال») في
`unit` لا بمقاس الرقم. والأعمدة تتبع العدد تلقائياً (من ١ إلى ٦).
