    ...
    <div className="space-y-8 animate-fade-in" dir="rtl">
      ...
      <div className="bg-white rounded-xl p-4 border border-slate-100 shadow-sm relative overflow-hidden">
        ...
        <div className="relative flex flex-col lg:flex-row justify-between items-start lg:items-center gap-5">
          <div className="flex items-center gap-3 w-full lg:w-auto">
            ...
            <div>
              <h1 className="text-base font-black text-slate-800 mb-0.5">{charityName} - إدارة المنح</h1>
              <p className="text-slate-500 text-[11px] font-bold">متابعة دقيقة لطلبات المنح ومراحل الاعتماد والإغلاق</p>
            </div>
          </div>
          
          <div className="flex gap-3 flex-wrap">
            <div className="bg-amber-50 text-amber-700 px-4 py-2 rounded-xl flex items-center gap-2 border border-amber-100 shadow-sm">
              <div className="bg-amber-100 p-1 rounded-md shrink-0"><FileText className="w-3.5 h-3.5" /></div>
            ...
            <div className="bg-emerald-50 text-emerald-700 px-4 py-2 rounded-xl flex items-center gap-2 border border-emerald-100 shadow-sm">
              <div className="bg-emerald-100 p-1 rounded-md shrink-0"><CheckCircle2 className="w-3.5 h-3.5" /></div>
              <div className="flex flex-col">
            ...
            <div className="bg-slate-50 text-slate-700 px-4 py-2 rounded-xl flex items-center gap-2 border border-slate-200 shadow-sm">
              <div className="bg-slate-200 p-1 rounded-md shrink-0"><Check className="w-3.5 h-3.5" /></div>
              <div className="flex flex-col">
      ...
      <div className="flex justify-between items-center bg-white p-2 rounded-2xl shadow-sm border border-slate-100 overflow-x-auto">
        <div className="flex gap-1">
          <button onClick={() => setActiveTab("PENDING")} className={`px-4 py-2 rounded-xl text-sm font-bold whitespace-nowrap transition-all ${activeTab === "PENDING" ? "bg-primary text-white shadow-md" : "text-slate-500 hover:bg-slate-50"}`}>
            المنح المرفوعة ({pendingGrants.length})
          </button>
          <button onClick={() => setActiveTab("APPROVED")} className={`px-4 py-2 rounded-xl text-sm font-bold whitespace-nowrap transition-all ${activeTab === "APPROVED" ? "bg-emerald-500 text-white shadow-md shadow-emerald-500/20" : "text-slate-500 hover:bg-slate-50"}`}>
            المنح المقبولة ({approvedGrants.length})
          </button>
          <button onClick={() => setActiveTab("CLOSED")} className={`px-4 py-2 rounded-xl text-sm font-bold whitespace-nowrap transition-all ${activeTab === "CLOSED" ? "bg-slate-700 text-white shadow-md" : "text-slate-500 hover:bg-slate-50"}`}>
            المنح المغلقة ({closedGrants.length})
          </button>
          <button onClick={() => setActiveTab("REJECTED")} className={`px-4 py-2 rounded-xl text-sm font-bold whitespace-nowrap transition-all ${activeTab === "REJECTED" ? "bg-red-500 text-white shadow-md shadow-red-500/20" : "text-slate-500 hover:bg-slate-50"}`}>
      ...
      {showAddGrant && (
        <form onSubmit={handleAddGrant} className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm animate-fade-in grid grid-cols-1 md:grid-cols-5 gap-4 items-end">
          <div className="md:col-span-1">
            <label className="block text-xs font-bold text-slate-500 mb-1.5">الجهة المانحة</label>
            {donorAccounts.length > 0 ? (
              ...
              <input required type="text" placeholder="اكتب اسم الجهة" value={grantForm.entityName} onChange={e => setGrantForm({...grantForm, entityName: e.target.value})} className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-2.5 text-sm outline-none focus:ring-2 focus:ring-primary font-bold" />
            )}
          </div>
          <div className="md:col-span-1">
            <label className="block text-xs font-bold text-slate-500 mb-1.5">اسم المشروع</label>
          ...
          <div className="md:col-span-1">
            <label className="block text-xs font-bold text-slate-500 mb-1.5">التكلفة الإجمالية (ريال)</label>
          ...
          <div className="md:col-span-1">
            <label className="block text-xs font-bold text-slate-500 mb-1.5">حالة الطلب الأولية</label>
            <select value={grantForm.status} onChange={e => setGrantForm({...grantForm, status: e.target.value})} className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-2.5 text-sm outline-none focus:ring-2 focus:ring-primary font-bold text-slate-700">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">
        {getFilteredGrants().map(grant => (
          <div key={grant.id} className="bg-white rounded-2xl border border-slate-200 shadow-sm hover:shadow-md transition-shadow flex flex-col relative overflow-hidden group">
            ...
            <div className="p-5 flex-1 space-y-4">
              <div className="flex justify-between items-start gap-2">
                <div>
                  <h3 className="font-black text-slate-800 text-lg leading-tight mb-1">{grant.initiativeName}</h3>
              ...
              <div className="bg-slate-50/50 rounded-xl p-3 border border-slate-100 grid grid-cols-2 gap-3">
                <div>
                  <p className="text-[10px] font-bold text-slate-400 mb-0.5">المبلغ المطلوب / الإجمالي</p>
                  <p className="text-sm font-black text-slate-700">{grant.requestedAmount.toLocaleString('en-US')} <span className="text-[10px]">ريال</span></p>
                </div>
                {grant.collectedAmount != null && (
                  <div>
                    <p className="text-[10px] font-bold text-slate-400 mb-0.5">التحصيل</p>
                    <p className="text-sm font-black text-emerald-600">{grant.collectedAmount.toLocaleString('en-US')} <span className="text-[10px]">ريال</span></p>
                  </div>
                )}
                {grant.beneficiariesCount != null && (
                  <div>
                    <p className="text-[10px] font-bold text-slate-400 mb-0.5">عدد المستفيدين</p>
                    <p className="text-sm font-black text-slate-700">{grant.beneficiariesCount.toLocaleString('en-US')}</p>
                  </div>
                )}
                {grant.installmentsCount != null && (
                  <div>
                    <p className="text-[10px] font-bold text-slate-400 mb-0.5">عدد الأقساط</p>
                    <p className="text-sm font-black text-slate-700">{grant.installmentsCount}</p>
                  </div>
                )}
              </div>
              ...
              {grant.installmentsNotes && (
                <div className="bg-blue-50/50 p-3 rounded-xl border border-blue-100">
                  <p className="text-[10px] font-bold text-blue-400 mb-1">ملاحظات الأقساط:</p>
                  <p className="text-xs font-bold text-blue-900 leading-relaxed">{grant.installmentsNotes}</p>
                </div>
              )}
              
              {grant.closureDate && (
                <div className="bg-slate-100 p-3 rounded-xl border border-slate-200 flex justify-between items-center">
                  <span className="text-[10px] font-bold text-slate-500">تاريخ الإغلاق:</span>
                  <span className="text-xs font-black text-slate-800">{new Date(grant.closureDate).toLocaleDateString('ar-SA')}</span>
                </div>
              )}
            </div>
              ...
              <div className="flex gap-2">
                <select 
                  ...
                  className={`flex-1 text-xs font-bold px-3 py-2.5 rounded-xl border-none outline-none appearance-none cursor-pointer focus:ring-2 focus:ring-primary/20 ${
                    grant.status === "APPROVED" ? "bg-emerald-100 text-emerald-800" :
                    grant.status === "REJECTED" ? "bg-red-100 text-red-800" :
                    grant.status === "CLOSED" ? "bg-slate-200 text-slate-800" :
                ...
                {grant.status === "APPROVED" && (
                  <button onClick={() => handleStatusChangeRequest(grant.id, "APPROVED")} className="px-3 bg-white border border-slate-200 text-slate-400 hover:text-primary hover:border-primary/30 rounded-xl transition-all">
        ...
        {getFilteredGrants().length === 0 && (
          <div className="col-span-full py-16 flex flex-col items-center justify-center text-slate-400 bg-white rounded-2xl border border-slate-100 border-dashed">
          ...
          <div className="bg-white rounded-3xl w-full max-w-lg overflow-hidden shadow-2xl border border-slate-100 flex flex-col max-h-[90vh]">
            <div className="p-6 md:p-8 overflow-y-auto custom-scrollbar">
              <div className="flex justify-between items-start mb-6">
                ...
                <button onClick={() => { setGrantApprovalModal({ isOpen: false, grantId: null }); }} className="p-2 bg-slate-50 text-slate-400 hover:text-slate-600 rounded-xl">
                  <X className="w-5 h-5" />
                </button>
              </div>
              
              <h3 className="text-xl font-black text-slate-800 mb-2">تأكيد قبول واعتماد المنحة</h3>
              ...
              <div className="space-y-4">
                <div>
                  <label className="block text-xs font-black text-slate-700 mb-1">المبلغ المعتمد النهائي (ريال) *</label>
                  <input
                    ...
                    className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-base outline-none focus:ring-2 focus:ring-emerald-500 font-black text-emerald-700"
                    placeholder="المبلغ"
                  />
                </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-xs font-black text-slate-700 mb-1">التحصيل (المبلغ المتحصل عليه)</label>
                    <input
                      type="number" min="0"
                      value={approvalForm.collectedAmount}
                      onChange={(e) => setApprovalForm({...approvalForm, collectedAmount: e.target.value})}
                      className="w-full bg-white border border-slate-200 rounded-xl px-4 py-3 text-sm outline-none focus:ring-2 focus:ring-emerald-500 font-bold"
                      placeholder="اختياري"
                    />
                  </div>
                  <div>
                    <label className="block text-xs font-black text-slate-700 mb-1">عدد المستفيدين</label>
                    <input
                      type="number" min="0"
                ...
                <div>
                  <label className="block text-xs font-black text-slate-700 mb-1">عدد الأقساط</label>
                  <input
                    type="number" min="1"
                ...
                <div>
                  <label className="block text-xs font-black text-slate-700 mb-1">ملاحظات الأقساط</label>
                  <textarea
                    rows={2}
                    value={approvalForm.installmentsNotes}
                    onChange={(e) => setApprovalForm({...approvalForm, installmentsNotes: e.target.value})}
                    className="w-full bg-white border border-slate-200 rounded-xl px-4 py-3 text-sm outline-none focus:ring-2 focus:ring-emerald-500 font-bold resize-none custom-scrollbar"
                    placeholder="أدخل أي ملاحظات حول جدولة الأقساط..."
                  />
                </div>
              </div>
            </div>
            <div className="bg-slate-50 p-4 flex gap-3 border-t border-slate-100 shrink-0">
              <button
                onClick={() => { setGrantApprovalModal({ isOpen: false, grantId: null }); }}
                className="flex-1 py-3.5 bg-white border border-slate-200 text-slate-700 font-bold rounded-xl hover:bg-slate-100 transition-colors"
                disabled={isPending}
              >
                إلغاء
              </button>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-fade-in">
          <div className="bg-white rounded-3xl w-full max-w-sm overflow-hidden shadow-2xl border border-slate-100">
            <div className="p-8 text-center">
              <div className="w-20 h-20 mx-auto bg-slate-50 text-slate-600 rounded-full flex items-center justify-center mb-6 shadow-inner border border-slate-200">
                <Check className="w-10 h-10" strokeWidth={3} />
              </div>
              <h3 className="text-xl font-black text-slate-800 mb-2">إغلاق المشروع</h3>
              ...
              <div className="text-right">
                <label className="block text-xs font-black text-slate-700 mb-1">تاريخ الإغلاق *</label>
                <input
                  type="date"
                  value={closureDate}
                  onChange={(e) => setClosureDate(e.target.value)}
                  className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-base outline-none focus:ring-2 focus:ring-slate-500 font-black text-slate-800 text-right"
                />
              </div>
            </div>
