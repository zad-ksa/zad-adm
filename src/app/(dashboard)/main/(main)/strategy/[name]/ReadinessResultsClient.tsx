...
import ReadinessPrintReport from "@/components/report/ReadinessPrintReport";
...
export default function ReadinessResultsClient({ responses }: { responses: any[] }) {
  ...
  return (
    <div className="space-y-8">
        ...
        <div className="space-y-4 max-h-[800px] overflow-y-auto pr-2 custom-scrollbar">
          {(showAllParticipants ? responses : responses.slice(0, 5)).map((res) => {
            ...
            return (
              <div 
                key={res.id} 
                className={`bg-white dark:bg-slate-900 rounded-xl p-4 border transition-all group ${isExcluded ? 'border-slate-200 dark:border-slate-700 opacity-60 bg-slate-50 dark:bg-slate-900/50' : 'border-slate-200 dark:border-slate-800 hover:border-primary/30 shadow-[0_1px_2px_rgb(15_23_42/0.04)] dark:bg-slate-900'}`}
              >
          ...
          {!showAllParticipants && responses.length > 5 && (
            <button
              ...
              className={cx(btn.secondary, "w-full")}
            >
      ...
      <ReadinessPrintReport
        charityName={responses[0]?.charityName || "الجمعية"}
