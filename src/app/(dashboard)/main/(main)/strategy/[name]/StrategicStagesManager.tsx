...
export default function StrategicStagesManager({ 
  charityId, 
  initialStages,
  timelineName = "المخطط الزمني للتخطيط الاستراتيجي",
  timelineDept = "STRATEGY"
}: { 
  charityId: string, 
  initialStages: Stage[],
  timelineName?: string,
  ...
  const handleAdd = () => {
    ...
    startTransition(async () => {
      ...
      await addStrategicStage(charityId, newName, "", newDescription, newStartDate, newEndDate, newIsContinuous, true);
      router.refresh();
    });
  };
  ...
  const handleUpdate = (id: string) => {
    ...
    startTransition(async () => {
      ...
      setEditingId(null);
      await updateStrategicStage(id, editName, "", editDescription, editStartDate, editEndDate, editIsContinuous, stageToUpdate.isActive);
    });
  };
  ...
  const handleDeleteConfirm = () => {
    ...
    startTransition(async () => {
      await deleteStrategicStage(stageId);
    });
  };
  ...
  const handleToggleActive = (id: string, currentActive: boolean) => {
    ...
    startTransition(async () => {
      await toggleActiveStrategicStage(id, !currentActive);
    });
  };
  ...
  const handleMove = (id: string, direction: 'up' | 'down') => {
    ...
    startTransition(async () => {
      ...
      await reorderStrategicStages(charityId, reorderedIds);
    });
  };
  ...
  const handleConfigUpdate = () => {
    ...
    startTransition(async () => {
      await updateTimelineConfig(charityId, "STRATEGY", configName, configDept);
  ...
  return (
    <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 shadow-[0_1px_2px_rgb(15_23_42/0.04)] overflow-hidden transition-colors">
      <div
      ...
      {isExpanded && (
        ...
        <div className="p-6 border-b border-slate-100 dark:border-slate-700 bg-slate-50 dark:bg-slate-900/30">
          <h4 className="font-semibold text-caption mb-4 text-slate-700 dark:text-slate-300">إعدادات المخطط الزمني</h4>
          <div className="flex flex-col sm:flex-row gap-4 mb-4">
            <div className="flex-1">
              ...
              <input 
                ...
                className="w-full border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-800 text-slate-800 dark:text-slate-100 rounded-lg px-3 py-2 text-caption focus:ring-2 focus:ring-primary/50 outline-none"
              />
            </div>
            <div className="flex-1">
