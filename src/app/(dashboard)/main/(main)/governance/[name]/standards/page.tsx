...
import { Scale, ChevronRight, FileText, CheckCircle2 } from "lucide-react";
import Link from "next/link";
import { redirect } from "next/navigation";

export default async function DashboardGovernanceStandardsPage({ 
  params,
  searchParams
}: { 
  ...
  if (!governanceManuals[sizeParam]) {
    redirect(`/main/governance/${encodeURIComponent(charityName)}`);
  }
  ...
  return (
    <div className="min-h-full bg-[#FAFAFA] dark:bg-[#0A0A0A] p-4 md:p-8 selection:bg-primary/20 selection:text-primary rounded-3xl" dir="rtl">
      ...
      <div className="max-w-5xl mx-auto mb-8">
        <Link 
          href={`/main/governance/${encodeURIComponent(charityName)}?change_size=true`}
      ...
      <div className="mb-12 max-w-5xl mx-auto space-y-6">
        ...
        <p className="text-slate-500 dark:text-slate-400 max-w-2xl font-medium leading-relaxed text-sm md:text-base">
          توضح هذه الصفحة المعايير الأساسية، والممارسات الدقيقة، والشواهد المطلوبة لإثبات الامتثال، مصممة للإدارة العليا للجمعية.
        </p>
      </div>
