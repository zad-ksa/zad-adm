...
export default function NewsFilterClient({
  ...
  return (
    <main className="flex-1 min-w-0 py-8 relative" dir="rtl">
      ...
      {/* Page Header */}
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-slate-800 dark:text-slate-100 dark:text-slate-100 mb-2">الأخبار والإنجازات</h1>
        <p className="text-slate-600 dark:text-slate-300 dark:text-slate-300 font-medium">تصفية واستعراض كافة الإنجازات والتقارير الإخبارية للجمعيات المتعاقد معها</p>
      </div>
        ...
        <div className="space-y-8">
          ...
          <div className="bg-white dark:bg-slate-800 dark:bg-slate-800 rounded-2xl p-6 border border-slate-100 dark:border-slate-700/50 dark:border-slate-800/80 dark:border-slate-800/80 shadow-sm">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              ...
              <div className="space-y-2">
                <label className="block text-xs font-bold text-slate-500 dark:text-slate-400 dark:text-slate-400 flex items-center gap-1.5">
          ...
          <div className="space-y-6">
            <div className="flex items-center justify-between">
              <h3 className="font-bold text-slate-800 dark:text-slate-100 dark:text-slate-100 text-lg">الأخبار والتقارير ({filteredNews.length})</h3>
            </div>
                ...
                return (
                  ...
                  >
                    <div>
                      ...
                      {/* News Title */}
                      <h4 className="font-bold text-slate-800 dark:text-slate-100 dark:text-slate-100 text-base mb-2 group-hover:text-primary transition-colors duration-300">
                        {item.title}
                      </h4>

                      {/* Description */}
                      <p className="text-sm text-slate-500 dark:text-slate-400 dark:text-slate-400 font-medium leading-relaxed mb-6">
                        {item.description}
                      </p>
                    </div>
            ...
            <div className="bg-white dark:bg-slate-800 dark:bg-slate-800 rounded-3xl border border-slate-100 dark:border-slate-700/50 dark:border-slate-800/80 dark:border-slate-800/80 shadow-2xl w-full max-w-lg overflow-hidden relative z-10 transform transition-all duration-300 scale-100 p-6 md:p-8 max-h-[90vh] overflow-y-auto" dir="rtl">
              <div className="flex items-center justify-between mb-6 border-b border-slate-100 dark:border-slate-700/50 dark:border-slate-800/80 dark:border-slate-800/80 pb-4">
                <div>
                  ...
                  <p className="text-xs text-slate-500 dark:text-slate-400 dark:text-slate-400 font-medium leading-relaxed mt-1.5">
              ...
              <form onSubmit={handleCreateNews} className="space-y-5">
                <div>
                  <label className="block text-xs font-bold text-slate-500 dark:text-slate-400 dark:text-slate-400 mb-2">الجمعيات المعنية (اختر جمعية أو أكثر)</label>
                  <div className="border border-slate-200 dark:border-slate-700 dark:border-slate-700 rounded-xl p-3 bg-slate-50 dark:bg-slate-900/50 dark:bg-slate-900/50 max-h-40 overflow-y-auto space-y-2 focus-within:ring-4 focus-within:ring-amber-500/10 focus-within:border-amber-500/30 transition-all">
                    {[
                      "إدارة زاد",
                      "عدة جمعيات",
                ...
                <div>
                  <label className="block text-xs font-bold text-slate-500 dark:text-slate-400 dark:text-slate-400 mb-2">القسم المعني</label>
                  <select
                    value={newsCategory}
                    onChange={(e) => setNewsCategory(e.target.value)}
                    required
                    className="w-full bg-slate-50 dark:bg-slate-900/50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-700 dark:border-slate-700 rounded-xl px-4 py-3 text-sm outline-none focus:ring-4 focus:ring-amber-500/10 focus:border-amber-500/30 text-slate-800 dark:text-slate-100 dark:text-slate-100 transition-all font-bold cursor-pointer"
                  >
                    <option value="الاستراتيجية">الاستراتيجية</option>
                    <option value="التقنية">التقنية</option>
                    <option value="تنمية الموارد">تنمية الموارد</option>
                ...
                <div>
                  <label className="block text-xs font-bold text-slate-500 dark:text-slate-400 dark:text-slate-400 mb-2">العنوان</label>
                  <input
                    type="text"
                    required
                    value={newsTitle}
                    ...
                    className="w-full bg-slate-50 dark:bg-slate-900/50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-700 dark:border-slate-700 rounded-xl px-4 py-3 text-sm outline-none focus:ring-4 focus:ring-amber-500/10 focus:border-amber-500/30 text-slate-800 dark:text-slate-100 dark:text-slate-100 transition-all font-medium"
                  />
                </div>

                <div>
                  <label className="block text-xs font-bold text-slate-500 dark:text-slate-400 dark:text-slate-400 mb-2">الوصف (اختياري)</label>
                  <textarea
                    value={newsDescription}
                    onChange={(e) => setNewsDescription(e.target.value)}
                    placeholder="تفاصيل إضافية عن الخبر..."
                    rows={4}
                    className="w-full bg-slate-50 dark:bg-slate-900/50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-700 dark:border-slate-700 rounded-xl px-4 py-3 text-sm outline-none focus:ring-4 focus:ring-amber-500/10 focus:border-amber-500/30 text-slate-800 dark:text-slate-100 dark:text-slate-100 transition-all font-medium resize-none"
                  />
                </div>

                <div>
                  <label className="block text-xs font-bold text-slate-500 dark:text-slate-400 dark:text-slate-400 mb-2">تاريخ الخبر (اختياري)</label>
                  <input
                    type="date"
                    value={newsDate}
                    onChange={(e) => setNewsDate(e.target.value)}
                    className="w-full bg-slate-50 dark:bg-slate-900/50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-700 dark:border-slate-700 rounded-xl px-4 py-3 text-sm outline-none focus:ring-4 focus:ring-amber-500/10 focus:border-amber-500/30 text-slate-800 dark:text-slate-100 dark:text-slate-100 transition-all font-medium cursor-pointer"
                  />
