    ...
    <div className="space-y-8">
      ...
      <div className="flex justify-end">
        <button 
          ...
          className="flex items-center gap-2 bg-primary text-white px-6 py-3 rounded-lg font-semibold hover:bg-primary/90 hover:-translate-y-0.5 transition-all shadow-sm active:translate-y-0"
        >
      ...
      {schedules.length === 0 ? (
        <div className="bg-white dark:bg-slate-900 rounded-xl p-8 border border-slate-200 dark:border-slate-800 text-center flex flex-col items-center justify-center min-h-[250px]">
          ...
          <h3 className="text-section font-semibold text-slate-800 dark:text-slate-200 mb-2">لا توجد جداول اجتماعات</h3>
            ...
            >
              ...
              <div className="flex items-start justify-between mb-4 relative z-10">
                <div className="space-y-1">
                  <h3 className="text-title font-semibold text-slate-900 dark:text-white group-hover:text-primary transition-colors">
                    {schedule.title}
                  </h3>
                  <div className="flex items-center gap-1.5 text-caption text-slate-500 dark:text-slate-400 font-medium bg-slate-50 dark:bg-slate-900/50 w-fit px-2.5 py-1 rounded-lg">
              ...
              <div className="grid grid-cols-2 gap-3 mb-6 relative z-10 mt-auto">
                <div className="bg-slate-50 dark:bg-slate-900/50 p-3 rounded-xl border border-slate-100 dark:border-slate-800/60">
                  <p className="text-caption font-semibold text-slate-400 mb-1">الأيام المتاحة</p>
                  <p className="text-section font-semibold text-slate-700 dark:text-slate-300">
                ...
                <div className="bg-primary/5 dark:bg-primary/10 p-3 rounded-xl border border-primary/10 dark:border-primary/20">
                  <p className="text-caption font-semibold text-primary/70 mb-1">المواعيد المحجوزة</p>
                  <p className="text-section font-semibold text-primary">
              ...
              {schedule.alternativeRequests && schedule.alternativeRequests.length > 0 && (
                <div className="flex items-center gap-2 text-caption font-semibold text-amber-600 dark:text-amber-400 bg-amber-50 dark:bg-amber-950/30 border border-amber-100 dark:border-amber-900/40 rounded-xl px-3 py-2 mb-4 relative z-10">
              ...
              <div className="flex items-center gap-2 pt-4 border-t border-slate-100 dark:border-slate-800 relative z-10">
                <button 
                  ...
                  className="flex-1 flex items-center justify-center gap-2 py-2.5 rounded-lg font-semibold text-caption bg-slate-900 dark:bg-white text-white dark:text-slate-900 hover:opacity-90 transition-opacity disabled:opacity-50"
                >
              ...
              {schedule.bookings && schedule.bookings.length > 0 && (
                <div className="mt-4 pt-4 border-t border-slate-100 dark:border-slate-800 relative z-10">
                  <p className="text-caption font-semibold text-slate-400 mb-2">أحدث الحجوزات:</p>
                  <div className="space-y-1.5 max-h-24 overflow-y-auto custom-scrollbar pr-1">
                    {schedule.bookings.slice(0, 3).map((booking: any) => (
                      <div key={booking.id} className="flex items-center justify-between text-caption bg-slate-50 dark:bg-slate-900/50 px-2 py-1.5 rounded-lg border border-slate-100 dark:border-slate-800/50">
                    ...
                    {schedule.bookings.length > 3 && (
                      <p className="text-caption text-center text-slate-400 pt-1">+{schedule.bookings.length - 3} حجوزات أخرى</p>
                    )}
                  </div>
                </div>
              )}
