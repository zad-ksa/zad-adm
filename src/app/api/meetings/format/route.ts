...
const SYSTEM_PROMPT = `أنت مساعد متخصص في تنسيق محاضر اجتماعات مؤسسة زاد التنموية.
...
---
*صدر هذا المحضر عن مؤسسة زاد التنموية*
...
export async function POST(req: NextRequest) {
  ...
  const { rawNotes, title, date, attendees, location } = await req.json();
  if (!rawNotes?.trim()) {
