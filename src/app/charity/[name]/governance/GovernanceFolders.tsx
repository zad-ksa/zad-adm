...
export default function GovernanceFolders({ charityId, regulations, isAdmin }: any) {
  ...
  const FolderGraphic = ({ title }: { title: string }) => (
    <div className="relative group flex flex-col items-center justify-center w-full h-full cursor-pointer hover:-translate-y-2 transition-transform duration-300">
      <div className="transform scale-[0.6] origin-bottom -mt-8">
      ...
      <p className="text-base font-bold text-slate-800 dark:text-slate-100 pt-2 transition-colors group-hover:text-amber-600 text-center max-w-[200px] leading-snug">{title}</p>
    </div>
  );
